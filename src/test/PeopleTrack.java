...
class PeopleTrack {
	...
	public static int countNewPersons(MatOfRect currentDetection, HeapList<MatOfRect> previousDetections,
			MatOfRect facesDetection) {
		int counter = 0;

		for (Rect person : currentDetection.toList()) {
			if (isNewPerson(person, (HeapList<MatOfRect>) previousDetections.clone())) {
	...
	public static boolean isNewPerson(Rect person, HeapList<MatOfRect> previousDetections) {
		boolean result = true;
	...
	private static boolean isNear(Rect person, Rect previousPerson, double ratio) {
		Point personCenter = new Point(person.x + person.width / 2, person.y + person.height / 2);
		Point previousCenter = new Point(previousPerson.x + previousPerson.width / 2,
				previousPerson.y + previousPerson.height / 2);

		Point maxAllowed = new Point(ratio * (person.width + previousPerson.width) / 2,
				ratio * (person.height + previousPerson.height) / 2);

		Point movement = new Point(previousCenter.x - personCenter.x, previousCenter.y - personCenter.y);
